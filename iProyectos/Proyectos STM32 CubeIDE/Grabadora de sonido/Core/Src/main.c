...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define BUFFER_SIZE 1632  // Tamaño del búfer de audio (ajusta según sea necesario)
uint16_t audioBuffer[BUFFER_SIZE];  // Búfer para almacenar las muestras de audio
uint32_t writeIndex = 0;  // �?ndice de escritura en el búfer de audio
uint32_t recordingTime = 0;  // Tiempo de grabación en milisegundos (ajusta a 10000 para 10 segundos)
uint8_t recordingFlag = 0;  // Indicador de estado de grabación
uint32_t lastTick = 0;  // Último tick para cálculo del tiempo transcurrido
